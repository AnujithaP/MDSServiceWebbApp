...
namespace MDSServiceWebbApp.Services
{
    public class MDSServices : IMDSServices
    {
        ...
        public MDSServices(IConfiguration configuration)
        {
        ...
        public async Task<Person> AddPerson(Person person)
        {
            ...
            aNewMember.Attributes = new List<MDSService.Attribute>() { };

            aNewMember.Attributes.Add(
                new MDSService.Attribute()
                {
        ...
        // Har försökt att söka med Attribut men får inte det att fungera (eg. .SearchTerm = "Personnummer = '198504131121') det kraschar!
        public async Task<bool> CheckPerson(string name)
        {
            ...
            getRequest.MembersGetCriteria.MemberType = MemberType.Leaf;
            getRequest.MembersGetCriteria.MemberReturnOption = MemberReturnOption.Data;
            getRequest.MembersGetCriteria.SearchTerm = "Name = '" + name + "'";
        ...
        private async Task<IEnumerable<EntityMembers>> EntityMetadataGet(string modelname, string entityName)
        {
            ...
            var metaFilter = new MDSService.MetadataResultOptions()
            {
                Models = MDSService.ResultType.Identifiers
            };

            var metadatareq = new MDSService.MetadataGetRequest(new MDSService.International(), Guid.NewGuid(), metaFilter, new MDSService.MetadataSearchCriteria());
            ...
            {
                var modelsFilter = new List<MDSService.Identifier>()
                {
                    new MDSService.Identifier()
                    {
                        Id = metadata.Metadata.Models.Where(m => m.Identifier.Name == modelname).FirstOrDefault().Identifier.Id
                    }
                };

                var modelEntitiesFilter = new List<MDSService.Identifier>()
                {
                    new MDSService.Identifier()
                    {
                        Name = entityName
                ...
                var modelmembersreq = new MDSService.ModelMembersGetRequest()
                {
                    International = new MDSService.International(),
                    ModelsGetCriteria = new MDSService.ModelMembersGetCriteria()
                    {
                        Models = modelsFilter,
                        Entities = modelEntitiesFilter,
                    },
                    ModelsResultCriteria = new MDSService.ModelMembersResultCriteria()
                    {
        ...
        private static object GetValue(MDSService.Attribute attribute)
        {
            object result = null;
